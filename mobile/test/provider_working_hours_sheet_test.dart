...
class _FakeProviderTransport implements ApiTransport {
  ...
  Future<ApiResponse> send(ApiRequest request) async {
    ...
    if (request.path == 'provider-applications/me') {
      return const ApiResponse(statusCode: 200, body: {'status': 'approved'});
    }
...
void main() {
  test(
    'ProviderAvailability.scheduleSummary formats various schedules correctly',
    () {
      ...
      expect(
        weekdays.scheduleSummary,
        'Monday to Friday, 09:00–17:00 Asia/Kolkata',
      );
      ...
      expect(
        monToSatCustom.scheduleSummary,
        'Monday to Saturday, 10:00–19:00 Asia/Kolkata',
      );
    },
  );

  testWidgets(
    'provider can open working hours sheet and save custom days and hours',
    (tester) async {
