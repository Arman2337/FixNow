...
void main() async {
  ...
  final files = dir.listSync(recursive: true).whereType<File>().where((f) => f.path.endsWith('_test.dart'));
  ...
  for (final file in files) {
    ...
    if (content.contains('ServiceRequestScreen(') && !content.contains('addressRepository:')) {
    ...
    if (content.contains('CustomerProfileScreen(') && !content.contains('addressRepository:')) {
      content = content.replaceAll(
          'CustomerProfileScreen(',
    ...
    if (content.contains('SavedAddressSelectorCard(') && !content.contains('addressRepository:')) {
      content = content.replaceAll(
          'SavedAddressSelectorCard(',
    ...
    if (content.contains('SubServiceCatalogScreen(') && !content.contains('api:')) {
      content = content.replaceAll(
          'SubServiceCatalogScreen(',
    ...
    if (content.contains('ServiceDiscoveryController(') && !content.contains('SubServiceRepository')) {
      content = content.replaceAll(
          'ServiceDiscoveryController(',
    ...
    if (content.contains('SavedAddressRepository.instance')) {
      content = content.replaceAll('SavedAddressRepository.instance', 'SavedAddressRepository(api: ApiTransport(MockClient()), accessToken: () async => \'\')');
      changed = true;
    }

    // AddEditAddressModalSheet.show
    if (content.contains('AddEditAddressModalSheet.show(ctx)')) {
      content = content.replaceAll('AddEditAddressModalSheet.show(ctx)', 'AddEditAddressModalSheet.show(ctx, SavedAddressRepository(api: ApiTransport(MockClient()), accessToken: () async => \'\'))');
      changed = true;
    }
    if (content.contains('AddEditAddressModalSheet.show(context)')) {
      content = content.replaceAll('AddEditAddressModalSheet.show(context)', 'AddEditAddressModalSheet.show(context, SavedAddressRepository(api: ApiTransport(MockClient()), accessToken: () async => \'\'))');
      changed = true;
    }

    if (changed) {
      if (!content.contains('import \'package:fixnow_mobile/api/api_client.dart\';')) {
        content = 'import \'package:fixnow_mobile/api/api_client.dart\';\n' + content;
      }
      if (!content.contains('import \'package:fixnow_mobile/features/services/sub_service_item.dart\';')) {
