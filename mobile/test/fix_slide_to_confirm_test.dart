...
void main() {
  testWidgets(
    'FixSlideToConfirm renders label and snaps back if drag threshold not met',
    (tester) async {
      bool confirmed = false;
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: FixSlideToConfirm(onConfirmed: () async => confirmed = true),
          ),
        ),
      );
      ...
      final handleFinder = find.byIcon(
  ...
  testWidgets('FixSlideToConfirm triggers callback on full drag', (
    tester,
  ) async {
    bool confirmed = false;
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: SizedBox(
            width: 300,
            child: FixSlideToConfirm(onConfirmed: () async => confirmed = true),
          ),
        ),
      ),
    );
