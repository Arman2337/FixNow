...
## Advisory price estimation and review signals (FN-060)

`GET /ai/price-estimate?serviceCategoryId=…` is a customer-authenticated, deterministic advisory read in `backend/src/ai/pricing/`. No model participates: per ADR-0014 a model never determines a price. The estimate anchors on the admin-published category price (`PUBLISHED`) or, with at least `PRICE_ESTIMATE_RULES.observedMinSamples` paid orders, an observed min/median/max band from real ledger data (`OBSERVED`); "price on request" categories abstain honestly. Every estimate carries an advisory notice; nothing persists and booking totals are unaffected.

Trust review signals gained two deterministic rules — `customer-cancellation-frequency-v1` and `provider-refund-frequency-v1` — alongside the existing FN-055 pair, all sharing one windowed-count/dedupe implementation in `TrustService`. All four are evaluated best-effort at their flow points (booking cancellation, complaint creation, refund creation); evaluation failures never fail the main flow, and every signal routes to human review through the existing admin trust endpoints. The versioned suite `backend/src/ai/evals/price-fraud-eval-v1.spec.ts` pins thresholds and enforces false-positive, explanation, privacy, uncertainty, money, determinism, and drift gates.
