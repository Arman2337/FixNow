...
export default async function UsersPage({ searchParams }: { searchParams: Promise<{ search?: string; cursor?: string }> }) {
  ...
  return <AdminShell environment={env.appEnvironment} roles={session.session.roles} current="Users">
    <header className="border-b border-[var(--color-border-default)] pb-6"><p className="m-0 text-sm font-semibold text-[var(--color-primary)]">Minimized account view</p><h1 className="mt-2 mb-0 text-3xl font-bold leading-10">Users</h1><p className="mt-2 mb-0 text-[var(--color-text-secondary)]">Search by opaque user ID. Contact details and credentials are intentionally excluded.</p></header>
    <form className="mt-6 flex flex-col gap-3 sm:flex-row" role="search"><label htmlFor="user-search" className="sr-only">Search user ID</label><input id="user-search" name="search" defaultValue={params.search} placeholder="User ID prefix" className="min-h-12 flex-1 rounded-xl border border-[var(--color-border-default)] bg-[var(--color-surface-secondary)] px-4"/><button className="min-h-12 rounded-xl bg-[var(--color-primary)] px-5 text-sm font-semibold text-[var(--color-on-primary)]">Search</button></form>
    <section aria-labelledby="users-heading" className="mt-6"><h2 id="users-heading" className="sr-only">User results</h2>{page.items.length === 0 ? <div className="rounded-2xl border border-[var(--color-border-default)] bg-[var(--color-surface-primary)] p-6"><h3 className="m-0 text-lg font-semibold">No users found</h3><p className="mt-2 mb-0 text-[var(--color-text-secondary)]">Check the opaque ID and try again.</p></div> : <div className="overflow-x-auto rounded-2xl border border-[var(--color-border-default)]"><table className="w-full min-w-3xl border-collapse text-left"><thead className="bg-[var(--color-surface-secondary)] text-sm text-[var(--color-text-secondary)]"><tr><th className="p-4">User ID</th><th className="p-4">Status</th><th className="p-4">Roles</th><th className="p-4">Created</th><th className="p-4"><span className="sr-only">Open</span></th></tr></thead><tbody>{page.items.map((user) => <tr key={user.id} className="border-t border-[var(--color-border-default)]"><td className="p-4 font-mono text-sm">{user.id}</td><td className="p-4"><StatusBadge status={user.status}/></td><td className="p-4 text-sm text-[var(--color-text-secondary)]">{user.roles.join(", ") || "No active role"}</td><td className="p-4 text-sm text-[var(--color-text-secondary)]">{new Date(user.createdAt).toLocaleDateString()}</td><td className="p-4"><Link className="font-semibold text-[var(--color-primary)]" href={`/users/${user.id}`}>View details</Link></td></tr>)}</tbody></table></div>}
