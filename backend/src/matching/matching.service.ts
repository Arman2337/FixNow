...
export class MatchingService {
  ...
  async findEligibleProviders(
    ...
    const query = this.profileRepository
      ...
      .innerJoin('provider_availability', 'availability', 'availability.user_id = profile.user_id')
      .innerJoin('provider_skills', 'skill', 'skill.user_id = profile.user_id')
      .where('user.status = :accountStatus', { accountStatus: AccountStatus.Active })
      .andWhere('availability.status = :availStatus', { availStatus: ProviderAvailabilityStatus.Online })
      .andWhere('skill.serviceCategoryId = :categoryId', { categoryId: serviceCategoryId })
      .andWhere('skill.isVerified = :isVerified', { isVerified: true })
      .andWhere(`${haversineSql} <= profile.serviceRadiusKm`, { lat: locationLat, lng: locationLng })
